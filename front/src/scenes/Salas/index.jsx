...
export default class Salas extends Component {
  ...
  goCadastrarAlterarProduto = () => {
    ...
    if (sala.nome === "" || sala.descricao === "") {
      return;
    }

    CadastrarAlteraSala.cadastrarAlterarSala(sala.id, sala.nome, sala.descricao)
      .then(result => {
        this.setState({
          id: "",
          nome: "",
  ...
  goDeletarSala = () => {
    DeletarSalaService.deletarSala(this.state.salaASerExcluida.id)
      .then(result => {
        this.setState({
          salaASerExcluida: {}
        });
  ...
  renderModal() {
    return (
      ...
      >
        ...
        <ModalBody>
          Você deseja realmente excluir a{" "}
          <b>{this.state.salaASerExcluida.nome}</b>?
        </ModalBody>
        <ModalFooter>
          <Button color="danger" onClick={this.goDeletarSala}>
            Excluir
          </Button>
          ...
          return (
            <tr>
              ...
              <td className="sala-coluna">
                <span
                  className="button-sala"
                  onClick={() => this.onClickAlterarSala(sala)}
                >
                  Editar
                </span>
                <br />
                <br />
                <span className="button-sala" onClick={() => this.toggle(sala)}>
                  Excluir
                </span>
              </td>
            </tr>
